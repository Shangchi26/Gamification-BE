...
@section('content')
    <section class="bg-gray-100 min-h-screen w-full pl-6 sm:pl-[88px] pr-6 overflow-hidden font-inter">
        <div class="flex w-full items-center gap-1 py-2 font-inter">
            ...
            <p class="text-gray-400">/ Update item</p>
        </div>
        <h1 class="font-bold text-2xl leading-7 my-6">Update item</h1>
        @if (session('success'))
            ...
            <form action="{{ route('item.update', $item->id) }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="w-full max-w-md py-2">
                    ...
                    <div class="w-full flex justify-between text-gray-900">
                        <div>
                            <input type="radio" id="1" name="rank" value="1" {{ $item->rank === '1' ? 'checked' : ''}}>
                            <label for="1">1</label>
                        </div>
                        <div>
                            <input type="radio" id="2" name="rank" value="2" {{ $item->rank === '2' ? 'checked' : ''}}>
                            <label for="2">2</label>
                        </div>
                        <div>
                            <input type="radio" id="3" name="rank" value="3" {{ $item->rank === '3' ? 'checked' : ''}}>
                            <label for="3">3</label>
                        </div>
                        <div>
                            <input type="radio" id="4" name="rank" value="4" {{ $item->rank === '4' ? 'checked' : ''}}>
                            <label for="4">4</label>
                        </div>
                        <div>
                            <input type="radio" id="5" name="rank" value="5" {{ $item->rank === '5' ? 'checked' : ''}}>
                            <label for="5">5</label>
                        </div>
                    </div>
                </div>
                ...
                <div>
                    ...
                    <div class="flex items-center justify-start">
                        <input type="checkbox" value="1" name="can_reduce" id="reduce" {{ $item->can_reduce === 1 ? 'checked' : ''}}
